...
export class GymAccessPageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (accessToken) {
      console.log('Access token preview:', `${accessToken.substring(0, 15)}...${accessToken.substring(accessToken.length - 10)}`);
    ...
    if (!isAuthenticated) {
      ...
      console.warn('User is not authenticated - loading with mock data or allowing unauthenticated calls');
      ...
      .subscribe({
        next: (response: GymAccessHistoryResponse) => {
          ...
          if (response && response.history && response.history.length > 0) {
            console.log('Sample record from response:', JSON.stringify(response.history[0], null, 2));
            console.log('Successful field details:', {
              value: response.history[0].successful,
              type: typeof response.history[0].successful,
              isBoolean: response.history[0].successful === true || response.history[0].successful === false,
          ...
          if (response) {
            ...
            this.hasError = true;
            this.errorMessage = 'Error al cargar el historial de accesos - no se recibió respuesta del servidor';
        ...
        error: (error) => {
          ...
          if (error.status === 401) {
            ...
            errorMessage = 'Servicio no encontrado. Verifique la configuración del servidor.';
  ...
  testBackendConnectivity(): void {
    ...
    this.gymAccessAdminService.testConnectivity()
      .subscribe({
        next: (result) => {
  ...
  testAuthentication(): void {
    ...
    this.gymAccessAdminService.testAuthentication()
      .subscribe({
        next: (result) => {
