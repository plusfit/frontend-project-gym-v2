...
export class ExchangeHistoryComponent implements OnInit {
  ...
  loadExchanges(): void {
    ...
    const statusValue = this.statusFilter.value as
      | "completed"
      | "pending"
      | "cancelled"
      | "all";
    ...
    this.exchangesService.getExchangeHistory(filters).subscribe({
      ...
      error: (error) => {
        this.showSnackBar(
          "Error",
          "Error al cargar el historial de canjes. Verifique que el backend esté funcionando.",
          "error",
        );
        this.data = [];
        this.totalItems = 0;
  ...
  onChangeStatus(exchange: Exchange): void {
    const availableStatuses = this.getAvailableStatusTransitions(
      exchange.status,
    );
    ...
    if (availableStatuses.length === 0) {
      this.showSnackBar(
        "Información",
        "No hay cambios de estado disponibles para este canje",
        "warning",
      );
      return;
    }
  ...
  private getAvailableStatusTransitions(
    currentStatus: string,
  ): Array<{ value: "completed" | "pending" | "cancelled"; label: string }> {
    switch (currentStatus) {
      ...
      case "completed":
        return [{ value: "cancelled", label: "Cancelar" }];
      case "cancelled":
  ...
  private confirmStatusChange(
    exchange: Exchange,
    newStatus: { value: "completed" | "pending" | "cancelled"; label: string },
  ): void {
    ...
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      data: {
        ...
        type: DialogType.CONFIRM_CHANGE,
      },
    });
      ...
      .subscribe({
        next: (response) => {
          if (response.success) {
            ...
            this.showSnackBar(
              "Error",
              "No se pudo actualizar el estado del canje",
              "error",
            );
          }
          this.loading = false;
        ...
        error: (error) => {
          ...
          const errorMessage =
            error?.error?.message || "Error al actualizar el estado del canje";
